var fileFlag  = require('constants').S_IFREG
...
function save(blobs, vinyl, opts, cb) {
  ...
  eos(vinyl.pipe(ws), function(err) {
    ...
    stat.mode = (opts.mode || 0777) | fileFlag
...
function decode(blobs, file, opts) {
  ...
  var cwd = file.cwd = path.normalize('/')
...
function decodeStat(plain) {
  ...
  stat.mode  = fileFlag | plain.mode // set regular file flag
